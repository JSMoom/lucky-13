...
prediction_grid <- expand.grid(
  session_scaled = seq(min(multilevel_data_unique$session_scaled), max(multilevel_data_unique$session_scaled), length.out = 20),
...
prediction_grid$predicted_q_score <- predict(intervention_2, newdata = prediction_grid, allow.new.levels = TRUE)

# Aggregate over all other unnecessary variables (for this plot)
prediction_summary <- prediction_grid %>%
  group_by(session_scaled, period) %>%
  summarize(predicted_q_score = mean(predicted_q_score), .groups = "drop")


### Main plot - Difference in Ability Growth Between the 2 Periods ###
ggplot(prediction_summary, aes(x = session_scaled, y = predicted_q_score, color = period)) +
  geom_line(aes(group = period), size = 1.2) +
  theme_minimal() +
  labs(
    title = "Difference in Growth Slope Over 20 Sessions: Sep-Oct VS Oct-Dec",
    x = "Session",
    y = "Predicted Q-Score",
    color = "Period"
  ) +
...
prediction_summary <- prediction_grid %>%
  group_by(period, domain_id, session_scaled) %>%
  summarize(predicted_q_score = mean(predicted_q_score), .groups = "drop")

for (i in domains){
  p <- ggplot(prediction_summary[prediction_summary$domain_id == i,], aes(x = session_scaled, y = predicted_q_score, color = period))+
...
prediction_summary <- prediction_grid %>%
  group_by(period, grade, session_scaled) %>%
  summarize(predicted_q_score = mean(predicted_q_score), .groups = "drop")

for (i in grades){
  p <- ggplot(prediction_summary[prediction_summary$grade == i,], aes(x = session_scaled, y = predicted_q_score, color = period)) +
    geom_line(aes(group = period), size = 1.2) +  # Fit line for each period
    theme_minimal() +
    labs(
...
prediction_summary <- prediction_grid %>%
  group_by(period, domain_id, grade, session_scaled) %>%
  summarize(predicted_q_score = mean(predicted_q_score), .groups = "drop")
...
for (i in grades) {
  for (j in domains) {
    ...
    # Create the plot
    p <- ggplot(prediction_summary[prediction_summary$grade == i & prediction_summary$domain_id == j,],
                aes(x = session_scaled, y = predicted_q_score, color = period)) +
      geom_line(aes(group = period), size = 1.2) +  # Fit line for each period
      theme_minimal() +
      labs(
