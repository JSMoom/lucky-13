...
In this section we look at the percentage of no answers before and after the implementation of the extended deadline.


```{r}
# Plot percentage of no answer by domain
...
# Plot percentage of no answer by grade
no_answer_by_grade <- data %>%
...
# Plot percentage of no answer by difficulty
no_answer_by_difficulty <- data %>%
...
# Plot percentage of no answer by learning goal
no_answer_by_learning_goal <- data %>%
...
grid.arrange(no_answer_by_domain, no_answer_by_grade, no_answer_by_difficulty, no_answer_by_learning_goal, ncol = 2)
```
### TESTING SIGNIFICANCE

Here we perform t-test to see if the observed patterns are significant.

```{r}
# Get mean, sd, and n of before and after
group_summary <- data %>%
  group_by(new_UI) %>%
  summarise(
...
In this section we look at how often students use the help function before and after the change. The plots show percentage compared to total answers.

```{r}
# Plot percentage of no help by domain
help_domain <- data %>%
  group_by(new_UI, domain_name) %>%
  summarise(
...
# Plot percentage of no help by grade
help_grade <- data %>%
  group_by(new_UI, grade) %>%
  summarise(
    help_percentage = mean(help) * 100,
...
# Plot percentage of no help by difficulty mode
help_difficulty <- data %>%
  group_by(new_UI, modeDifficulty) %>%
  summarise(
    help_percentage = mean(help) * 100,
    sd = sd(help) * 100,
...
# Plot percentage of no help by learning goal
help_learning_goal <- data %>%
  group_by(new_UI, learning_goal) %>%
  summarise(
    help_percentage = mean(help) * 100,
    sd = sd(help) * 100,
...
# Combine all plots into a 2x2 grid
grid.arrange(help_domain, help_grade, help_difficulty, help_domain, ncol = 2)
```

### TESTING SIGNIFICANCE

Again we perform t-tests to see whether the observed patterns are significant.
...
Here we look at the percentage of correct answers before and after the change.
```{r}
# Compute percentage of correct answers by domain
...
# Plot percentage of correct answers per domain
correct_domain <- ggplot(correct_answers_domain, aes(x = correct_percentage, y = domain_name, fill = UI_Status)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.8)) +
  scale_fill_manual(values = custom_colors) +
  labs(title = "    By Domain", x = "   ", y = "Domain", fill = "Update on Oct 25th") +
  custom_theme

# Compute percentage of correct answers by grade
...
# Plot percentage of correct answers by grade
correct_grade <- ggplot(correct_answers_grade, aes(x = factor(grade), y = correct_percentage, fill = UI_Status)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.8)) +
  scale_fill_manual(values = custom_colors) +
  labs(title = "    By Grade", x = "Grade", y = "   ", fill = "Update on Oct 25th") +
  custom_theme

# Compute percentage of correct answers by difficulty
...
# Plot percentage of correct answers by difficulty
correct_difficulty <- ggplot(correct_answers_difficulty,
                             aes(x = Difficulty, y = correct_percentage, fill = UI_Status)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.8)) +
  scale_fill_manual(values = custom_colors) +
  labs(title = "    By Difficulty", x = "Difficulty", y = "   ", fill = "Update on Oct 25th") +
  custom_theme

# Compute percentage of correct answers by learning goal
...
# Plot correct answers by learning goal
correct_learning_goals <- ggplot(correct_answers_learning,
                                 aes(x = LearningGoal, y = correct_percentage, fill = UI_Status)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.8)) +
  scale_fill_manual(values = custom_colors) +
...
# Plot all together
grid.arrange(correct_domain, correct_grade, correct_difficulty, correct_learning_goals, ncol = 2)
```

### TESTING SIGNIFICANCE

We perform t-tests to see if the patterns are significant. 
```{r}
...
We compute the total number of items played before and after and perform a t-test to see if the differences are significant.
```{r}
...
We check if the average response time has changed significantly.
```{r}
...
We see if there is a difference in how many daily users are using the platform.
```{r}
...
We see if there is a difference in the average daily sessions before and after the change.
```{r}
...
Here we look at patterns in the data based on time. For instance we look at the rate of extended deadline use per daily hour, week since implementation and day of the week. A late response, as we call it here, means that the user provided a answer (meaning it was not no answer) within the extended deadline. 
```{r}
...
Again we test whether these patterns are significant this time using ANOVA. We also perform post-hoc tests.
```{r}
...
As domain 59 was of special interest we create the inital plots again this time for the different deadlines within the domain 59.
...
# Plot the result
no_answer_plot <- ggplot(result_no_answer,
                         aes(x = as.factor(deadline), y = percentage_no_answer, fill = as.factor(new_UI))) +
  geom_bar(stat = "identity", position = "dodge") +
...
# Plot the result using ggplot
correct_plot <- ggplot(correct_answered,
                       aes(x = as.factor(deadline), y = percentage_correct, fill = as.factor(new_UI))) +
  geom_bar(stat = "identity", position = "dodge") +
  scale_fill_manual(values = c("#00BFC4", "#fe6c66")) + # Custom colors for new_UI 0 and 1
  labs(
...
Here we plot the total number of items played across domains before and after the change. 
```{r}
...
items_played_plot <- ggplot(result_items_played,
                            aes(x = domain_name, y = total_items_played, fill = as.factor(new_UI))) +
  geom_bar(stat = "identity", position = "dodge") +
...
### PERCENTAGE OF LATE RESPONSES PAST EXTENDED DEADLINE

Here we compute the percentage of late responses that went past the extended deadline
```{r}
# Percent of late responses that went past extended
sum(ed_logs_after$answer == "…") / sum(ed_logs_after$late_response == 1) * 100
```
