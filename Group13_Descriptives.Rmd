...
data$new_UI <- ifelse(data$date > reference_date, "After", "Before")
...
plot1 <- data %>%
  ...
  summarise(
    ...
    sd = sd(no_answer) * 100,  # Use sd to calculate standard deviation
    .groups = 'drop'
  ) %>%
...
plot1 <- data %>%
  ...
  summarise(
    ...
    sd = sd(help) * 100,  # Use sd to calculate standard deviation
    .groups = 'drop'
  ) %>%
...
# Add "Before" and "After" labels to the new_UI column
data$new_UI <- ifelse(data$date > reference_date, "After", "Before")

# Calculate correct answers percentage by domain with UI status
correct_answers_domain <- data %>%
  ...
  ungroup() %>%
  mutate(
    UI_Status = ifelse(new_UI == "Before", "Before", "After")  # Label as "Old" or "New"
  )

correct_answers_domain$domain_name <- domain_names[as.character(correct_answers_domain$domain_id)]

plot1 <- ggplot(correct_answers_domain, aes(x = correct_percentage, y = domain_name, fill = UI_Status)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.8)) +
  labs(
    title = "Percentage of Correct Answers by Domain",
...
# Calculate correct answers percentage by grade
correct_answers_grade <- data %>%
  ...
  ungroup() %>%
  mutate(
    UI_Status = ifelse(new_UI == "Before", "Before", "After")
  )

plot2 <- ggplot(correct_answers_grade, aes(x = grade, y = correct_percentage, fill = UI_Status)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.8)) +
  labs(
    title = "Percentage of Correct Answers by Grade",
    x = "Grade",
...
# Calculate correct answers percentage by difficulty level
...
plot3 <- ggplot(correct_answers_difficulty, aes(x = Difficulty, y = correct_percentage, fill = UI_Status)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.8)) +
  labs(
    title = "Percentage of Correct Answers by Difficulty",
    x = "Difficulty",
    y = "Percentage of Correct Answers",
    fill = "Update on Oct 25th"
  ) +
...
# Calculate correct answers percentage by learning goal
correct_answers_learning <- data %>%
  ...
  mutate(
    UI_Status = ifelse(new_UI == "Before", "Before", "After"),
    LearningGoal = ifelse(learning_goal == 1, "Yes", "No")
  )

plot4 <- ggplot(correct_answers_learning, aes(x = Learning_Goal, y = correct_percentage, fill = UI_Status)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.8)) +
  labs(
    title = "Percentage of Correct Answers by Learning Goal",
    x = "Learning Goal",
    y = "Percentage of Correct Answers",
    fill = "Update on Oct 25th"
  ) +
  theme_minimal() +
  scale_fill_manual(values = c("Before" = "#57b0b5", "" = "#fe6c66")) +
  theme(
    plot.title = element_text(hjust = 0.5, size = 14, face = "bold"),
    legend.position = "bottom",
    legend.title = element_text(face = "bold"),
...
# Calculate correct answers percentage by domain with UI status and SD
correct_answers_domain <- data %>%
  group_by(new_UI, domain_id) %>%
  summarise(
    total_responses = n(),
    correct_responses = sum(correct_answered == "1"),
    correct_percentage = mean(correct_answered == "1") * 100,
    correct_sd = sd(correct_answered == "1") * 100  # Standard deviation
  ) %>%
  ungroup() %>%
  mutate(
    UI_Status = ifelse(new_UI == "Before", "Before", "After"),
...
plot1 <- ggplot(correct_answers_domain, aes(x = correct_percentage, y = domain_name, fill = UI_Status)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.8)) +
  geom_errorbar(aes(
    xmin = correct_percentage - correct_sd,
    xmax = correct_percentage + correct_sd
  ),
...
# ----------------------------------------------------------------------------
# Calculate correct answers percentage by grade with SD
correct_answers_grade <- data %>%
  group_by(new_UI, grade) %>%
  summarise(
    total_responses = n(),
  ...
  ungroup() %>%
  mutate(UI_Status = ifelse(new_UI == "Before", "Before", "After"))

plot2 <- ggplot(correct_answers_grade, aes(x = grade, y = correct_percentage, fill = UI_Status)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.8)) +
  geom_errorbar(aes(
    ymin = correct_percentage - correct_sd,
    ymax = correct_percentage + correct_sd
  ),
  position = position_dodge(width = 0.8),
  width = 0.2
  ) +
...
# ----------------------------------------------------------------------------
# Calculate correct answers percentage by difficulty with SD
correct_answers_difficulty <- data %>%
  group_by(new_UI, modeDifficulty) %>%
  summarise(
    total_responses = n(),
...
# ----------------------------------------------------------------------------
# Calculate correct answers percentage by learning goal with SD
correct_answers_learning <- data %>%
  group_by(new_UI, learning_goal) %>%
  summarise(
    total_responses = n(),
...
plot_week <- ggplot(late_response_by_week, aes(x = week_since_ui, y = late_response_rate)) +
  ...
  labs(
    title = "Late Responses by Week Since New UI",
    x = "Week Since New UI",
    y = "Late Response Rate (%)"
  ) +
  theme_minimal()

# 4. Late Responses by Days Since New UI
late_response_by_day <- filtered_data %>%
  mutate(days_since_ui = as.numeric(difftime(date, min(date), units = "days"))) %>%
  group_by(days_since_ui) %>%
  summarize(
    total_responses = n(),
    late_responses = sum(late_response),
    late_response_rate = round(mean(late_response) * 100, 2),
...
plot_day <- ggplot(late_response_by_day, aes(x = days_since_ui, y = late_response_rate)) +
  geom_line(color = "#fe6c66", size = 1) +
  geom_point(color = "#fe6c66", size = 2) +
  labs(
    title = "Late Responses by Days Since New UI",
    x = "Days Since New UI",
    y = "Late Response Rate (%)"
  ) +
  theme_minimal()
...
grid.arrange(plot_hour, plot_weekday, plot_week, plot_day, ncol = 2)
